...
public class PoData(
    ...
    public companion object {
         ...
         */
        public fun read(input: String): PoData {
            ...
            val iterator = input.splitToSequence("\n").iterator()
        ...
        private fun entry(
            entries: MutableMap<String, PoEntry>,
            context: String?,
            id: String?,
            str: String,
            plural: String?,
            ...
            if (!id.isNullOrEmpty() && (str.isNotEmpty() || cases.isNotEmpty())) {
                entries[if (context != null) "$context$CONTEXT_DELIMITER$id" else id] =
                    PoEntry(str, plural, cases.toTypedArray())
            }
        }
    }
}
